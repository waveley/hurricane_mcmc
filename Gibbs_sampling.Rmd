...
testB = res$B[[1000]]
testmu = res$mu[[1000]]
testsigmasq = res$sigmasq[[1000]]
testSIGMA = res$SIGMA[[1000]]
...
for (i in burn:chain_length) {
  cur_B[i-burn+1] = res$B[[i]][1,1]
}

cur_B %>% acf()
cur_B %>% ts.plot(xlab = "iteration", ylab = expression(beta[0]), main = expression(paste("Time Series Plot of ", beta[0])))
data.frame(cur_B) %>% ggplot(aes(x = cur_B)) + geom_histogram(bins = 50)

#mu
cur_mu <- NA
for (i in burn+1:chain_length) {
  cur_mu[i-burn] = res$mu[[i]][1]
}
